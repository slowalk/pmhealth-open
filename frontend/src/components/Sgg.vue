    ...
    export default {
        ...
        methods: {
                ...
                this.axios.get('/sgg_map', {params: {sd_cd: los}})
                    .then(res => {
                        ...
                        var d = this.geoJsonData.features.filter(function(g){
                            return g.properties.code.substring(0,2) == los && g.properties.name == sdVal.codeToCenterLocation[g.properties.code.substring(0,2)]
                        })[0]
                    ...
                    map_g.selectAll("path")
                        ...
                        .on('mouseenter', function(d) {
                            ...
                            var apd = airpol_data.filter((item) => {
                                    return String(item.SGG_CD) === String(d.properties.code);
                                })[0][t];
            ...
            drawBarChart (t, r, l) {
                ...
                var adbc = all_death_bar_chart_data;
                var k = this.geoJsonData.features.filter(function(g){
                        return String(g.properties.code) === String(that.get_sgg);
                    })[0]
                // var adbc_data = adbc.filter((item) => {

  ...
  mounted() {
    ...
    label_g.selectAll("rect")
      ...
      .attr("width", function(d) { 
        ...
        return (label_padding_width * 2 + 8.7 * (String(Math.round(result.burden)).length + 1))
      })
    ...
    label_g.selectAll("text.SD_NM")
      ...
      .attr("x", function(d) {
        ...
        else if("부산광역시" == d.properties.SIDO_NM) return (path.centroid(d)[0] + label_padding_width + 7);
        else if("제주특별자치도" == d.properties.SIDO_NM) return (path.centroid(d)[0] - 13);
        
        
        else if("울산광역시" == d.properties.SIDO_NM) return path.bounds(d)[1][0] + label_padding_width + textCenter - 2;
        ...
        else if("광주광역시" == d.properties.SIDO_NM) return path.bounds(d)[0][0] - 33 + label_padding_width + textCenter;
        ...
        else if(["강원도", "경상북도", "전라북도", "경상남도", "제주특별자치도", "충청북도"].includes(d.properties.SIDO_NM)) return path.centroid(d)[0]+ label_padding_width + textCenter - 23
        
        else if(["대전광역시", "대구광역시", "전라남도"].includes(d.properties.SIDO_NM)) return path.centroid(d)[0] + label_padding_width + textCenter - 3
        else return path.centroid(d)[0] + label_padding_width + textCenter

      })
      .attr("y", function(d) { 
        if("서울특별시" == d.properties.SIDO_NM) return path.bounds(d)[0][1] - 30 + fontSize/2 + label_padding_height
        else if("경기도" == d.properties.SIDO_NM) return path.centroid(d)[1] + 15 + fontSize/2 + label_padding_height
        // if("충청북도" == d.properties.SIDO_NM) return path.bounds(d)[0][1] + fontSize/2 + label_padding_height
        else if("세종특별자치시" == d.properties.SIDO_NM) return path.bounds(d)[0][1] + fontSize/2 + label_padding_height - 20
        else if("경상북도" == d.properties.SIDO_NM) return path.centroid(d)[1] + fontSize/2 + label_padding_height - 20
        else if("대구광역시" == d.properties.SIDO_NM) return path.centroid(d)[1] - 30+ fontSize/2 + label_padding_height
        else if("울산광역시" == d.properties.SIDO_NM) return path.bounds(d)[0][1] + fontSize/2 + label_padding_height
        // if("전라북도" == d.properties.SIDO_NM) return path.centroid(d)[1] + fontSize/2 + label_padding_height - 10
        // if(["부산광역시", "경상남도", "전라남도"].includes(d.properties.SIDO_NM)) return path.bounds(d)[1][1]  + fontSize/2 + label_padding_height
        else if(["강원도", "전라북도", "경상남도", "제주특별자치도", "충청북도"].includes(d.properties.SIDO_NM)) return path.centroid(d)[1] + fontSize/2 + label_padding_height - 20
    ...
    label_g.selectAll("text.dataNum")
      ...
      .attr("y", function(d) { 
        ...
        if("경상북도" == d.properties.SIDO_NM) return path.centroid(d)[1] + fontSize + label_padding_height*5 - 20
        ...
        // if("전라북도" == d.properties.SIDO_NM) return path.centroid(d)[1] + fontSize + label_padding_height*5 - 10
        // if(["부산광역시", "경상남도", "전라남도"].includes(d.properties.SIDO_NM)) return path.bounds(d)[1][1]  + fontSize/2 + label_padding_height
